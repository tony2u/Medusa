...
void SirenCppGenerator::SetTemplateName(StringRef val)
{
	ISirenGenerator::SetTemplateName(val);
	//load template
	mHeaderTemplate = FileSystem::Instance().ReadAllText(mTemplateName + ".h");
	mBodyTemplate = FileSystem::Instance().ReadAllText(mTemplateName + ".cpp");
}
...
bool SirenCppGenerator::GenerateType(BaseSirenCustomType& sirenClass) const
{
	...
	else
	{
		...
		{
			customClass.MutableAttribute().RemoveMode(SirenClassGenerateMode::Embeded);
			customClass.MutableAttribute().AddMode(SirenClassGenerateMode::Generate);
...
HeapString SirenCppGenerator::OnClassHeader(SirenCustomClass& sirenClass)const
{
	auto typeName = sirenClass.Name();
	HeapString sb;
	StringRef classOrStruct = sirenClass.Attribute().IsStruct() ? "struct" : "class";
	if (!sirenClass.Attribute().IsEmbeded())
	{
		if (sirenClass.BaseType() == nullptr)
		{
			if (sirenClass.Attribute().Has(SirenClassGenerateMode::SirenConfig))
			{
				sb.AppendFormat("{} {} :  ISirenConfig<{}>\n", classOrStruct,typeName, typeName);
			}
			else
			{
				sb.AppendFormat("{} {}\n", classOrStruct,typeName);
			}
		}
		else
		{
			if (sirenClass.Attribute().Has(SirenClassGenerateMode::SirenConfig))
			{
				sb.AppendFormat("{} {} :  {}, ISirenConfig<{}>\n", classOrStruct,typeName, sirenClass.BaseType()->Name(), typeName);
			}
			else
			{
				sb.AppendFormat("{} {} :  {}\n", classOrStruct, typeName, sirenClass.BaseType()->Name());
...
HeapString SirenCppGenerator::OnClassMethods(SirenCustomClass& sirenClass)const
{
	...
	for (auto sirenFieldPair : sirenClass.Fields())
	{
		...
		{
		case SirenFieldMethodType::Value:
			if (field->Type()==&SirenMachine::Instance().String())
			{
				sb.AppendFormat("\tSIREN_STRING({0}, {1});\n", GetTypeName(field->Type()), field->Name());
			}
			else
			{
...
HeapString SirenCppGenerator::OnClassSchema(SirenCustomClass& sirenClass)const
{
	...
	//add attributes
	for (auto& attr: sirenClass.Attribute().KeyValues())
	{
		auto& name=attr.Key;
		auto& val = attr.Value;
		//type str
		StringRef typeStr = StringPropertySet::GetDeclareType(val);
		if (typeStr=="char*")
		{
			sb.AppendFormat("\tconstexpr static {} {}{{ \"{}\" }};\n", typeStr, name, val);
		}
		else
		{
			sb.AppendFormat("\tconstexpr static {} {}{{ {} }};\n", typeStr, name, val);
