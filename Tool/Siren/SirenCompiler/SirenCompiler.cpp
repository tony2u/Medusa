...
#include "Generator/SirenSharpGenerator.h"
...
Usage:sc
 ...
 -t <template> --template=<template>  use template
	...
	if (args["create"].asBool())
	{
		...
		auto dir = Path::GetDirectory(assemblyName);
		if (!dir.IsEmpty())
		{
			FileSystem::Instance().AddDirectory(dir);
		}
	...
	else if (args["gen"].asBool())
	{
		...
		{
			SirenSharpGenerator generator(*assembly);
			generator.SetOutputPath(outdirName);
			generator.SetTemplateName(templateName);
			generator.SetTypeName(className);
			generator.Run();
	...
	else if (args["merge"].asBool())
	{
		...
		uint count = (uint)assemblyList.size();
