...
template <typename T>
struct LuaTypeMapping <Scale2<T>>//[IGNORE_PRE_DECLARE]
{
	static void Push(lua_State* L, const Scale2<T>& val)
	{
		LuaStack s(L);
		s.NewTable(0, 2);
		s.Rawset("X", val.X);
	...
	static Scale2<T> Get(lua_State* L, int index)
	{
		Scale2<T> result;
		LuaStack s(L);
	...
	static Scale2<T> Optional(lua_State* L, int index, const Scale2<T>& def)
	{
		return lua_isnoneornil(L, index) ? Scale2<T>::Zero : Get(L, index);
	}

	static bool TryGet(lua_State* L, int index, Scale2<T>& outValue)
	{
