...
namespace Siren
{
	...
	void Serialize(TWriter& writer, const Point2<T>& obj)
	{
		...
		writer.template OnField<T, false>(StringRef("X", 1), 0, obj.X);
		writer.template OnField<T, false>(StringRef("Y", 1), 1, obj.Y);
		writer.OnStructEnd();
	}
	...
	bool DeserializeTo(TReader& reader, Point2<T>& outObj)
	{
		...
		reader.template OnField<T, false>(StringRef("X", 1), 0, outObj.X);
		reader.template OnField<T, false>(StringRef("Y", 1), 1, outObj.Y);
		reader.OnStructEnd();
...
template <typename T>
struct LuaTypeMapping <Point2<T>>//[IGNORE_PRE_DECLARE]
{
	static void Push(lua_State* L, const Point2<T>& val)
	{
		LuaStack s(L);
		s.NewTable(0, 2);
		s.Rawset("X", val.X);
	...
	static Point2<T> Get(lua_State* L, int index)
	{
		Point2<T> result;
		LuaStack s(L);
	...
	static Point2<T> Optional(lua_State* L, int index, const Point2<T>& def)
	{
		return lua_isnoneornil(L, index) ? Point2<T>::Zero : Get(L, index);
	}

	static bool TryGet(lua_State* L, int index, Point2<T>& outValue)
	{
