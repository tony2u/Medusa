...
namespace Siren
{
	...
	void Serialize(TWriter& writer, const Rect2<T>& obj)
	{
		...
		writer.template OnField<Point2<T>, false>(StringRef("Origin",6), 0, obj.Origin);
		writer.template OnField<Size2<T>, false>(StringRef("Size", 4), 1, obj.Size);
		writer.OnStructEnd();
	}
	...
	bool DeserializeTo(TReader& reader, Rect2<T>& outObj)
	{
		...
		reader.template OnField<Point2<T>, false>(StringRef("Origin", 6), 0, outObj.Origin);
		reader.template OnField<Size2<T>, false>(StringRef("Size", 4), 1, outObj.Size);
		reader.OnStructEnd();
...
template <typename T>
struct LuaTypeMapping <Rect2<T>>//[IGNORE_PRE_DECLARE]
{
	static void Push(lua_State* L, const Rect2<T>& val)
	{
		LuaStack s(L);
		s.NewTable(0, 2);
		s.Rawset("Origin", val.Origin);
		s.Rawset("Size", val.Size);
	}

	static Rect2<T> Get(lua_State* L, int index)
	{
		Rect2<T> result;
		LuaStack s(L);
		result.Origin = s.Get<T>("Origin",index);
		result.Size = s.Get<T>("Size",index);
		return result;
	}

	static Rect2<T> Optional(lua_State* L, int index, const Rect2<T>& def)
	{
		return lua_isnoneornil(L, index) ? Rect2<T>::Zero : Get(L, index);
	}

	static bool TryGet(lua_State* L, int index, Rect2<T>& outValue)
	{
