...
Matrix4& Matrix4::SkewXY(const Rotation2F& skew)
{
	float tanX = Math::Tan(skew.X);
	float tanY = Math::Tan(skew.Y);
...
Matrix4& Matrix4::SkewX(float radian)
{
	float tanX = Math::Tan(radian);
	M12 = M11*tanX + M12;
	M22 = M21*tanX + M22;
	M32 = M31*tanX + M32;
	M42 = M41*tanX + M42;
...
Matrix4& Matrix4::SkewY(float radian)
{
	float tanY = Math::Tan(radian);
...
Matrix4& Matrix4::ResetWorld(const Scale3F& scale,const Rotation2F& skewXY, const Rotation3F& rotation, const Point3F& translation, FlipMask flip /*= FlipMask::None*/, Point3F flipPivot /*= Point3F::Zero*/, const Point3F& pivot/*=Point3F::Zero*/)
{
	*this = CreateWorld(scale, skewXY, rotation, translation, flip, flipPivot, pivot);
	return *this;
}

Matrix4& Matrix4::ResetWorld(const Size3F& size, const Point3F& anchor, const Scale3F& scale,const Rotation2F& skewXY, const Rotation3F& rotation, const Point3F& translation, FlipMask flip /*= FlipMask::None*/)
{
	*this = CreateWorld(size, anchor, scale, skewXY,rotation, translation, flip);
	return *this;
}

Matrix4& Matrix4::ResetWorldWithoutAnchor(const Scale3F& scale,const Rotation2F& skewXY, const Rotation3F& rotation, const Point3F& translation, FlipMask flip /*= FlipMask::None*/)
{
	*this = CreateWorldWithoutAnchor(scale, skewXY,rotation, translation, flip);
	return *this;
}
...
Matrix4 Matrix4::CreateSkewXY(const Rotation2F& skew)
{
	float tanX = Math::Tan(skew.X);
	float tanY = Math::Tan(skew.Y);

	return Matrix4(1, tanY, 0.f, 0.f,
		tanX, 1, 0.f, 0.f,
		0.f, 0.f, 1.f, 0.f,
		0.f, 0.f, 0.f, 1.f);
}
...
Matrix4 Matrix4::CreateSkewX(float radian)
{
	float tanX = Math::Tan(radian);

	return Matrix4(1, 0.f, 0.f, 0.f,
		tanX, 1, 0.f, 0.f,
		0.f, 0.f, 1.f, 0.f,
		0.f, 0.f, 0.f, 1.f);
}

Matrix4 Matrix4::CreateSkewY(float radian)
{
	float tanY = Math::Tan(radian);
...
Matrix4 Matrix4::CreateWorld(const Size3F& size, const Point3F& anchor, const Scale3F& scale,const Rotation2F& skewXY, const Rotation3F& rotation, const Point3F& translation, FlipMask flip /*= FlipMask::None*/)
{
	Point3F pivot(anchor.X*size.Width, anchor.Y*size.Height, anchor.Z*size.Depth);
	float flipX = MEDUSA_FLAG_HAS(flip, FlipMask::X) ? 1.f : 0.f;
	float flipY = MEDUSA_FLAG_HAS(flip, FlipMask::Y) ? 1.f : 0.f;
	float flipZ = MEDUSA_FLAG_HAS(flip, FlipMask::Z) ? 1.f : 0.f;



	Point3F flipPivot(anchor.X*size.Width*flipX, anchor.Y*size.Height*flipY, anchor.Z*size.Depth*flipZ);

	return CreateWorld(scale, skewXY,rotation, translation, flip, flipPivot, pivot);
}

Matrix4 Matrix4::CreateWorld(const Scale3F& scale,const Rotation2F& skewXY, const Rotation3F& rotation, const Point3F& translation, FlipMask flip /*= FlipMask::None*/, Point3F flipPivot /*= Point3F::Zero*/, const Point3F& pivot/*=Point3F::Zero*/)
{
	//matrix order: -flipPivot*flip*(flipPivot)*(-pivot)*scale*skew*rotation*translation

	/*

	-flipPivot*flip*(flipPivot)*(-pivot):

	(flipX, 0.f, 0.f, 0.f,
		0.f, flipY, 0.f, 0.f,
		0.f, 0.f, flipZ, 0.f,
		flipPivot.X*(1.f - flipX) - pivot.X, flipPivot.Y*(1.f - flipY)  - pivot.Y, flipPivot.Z*(1.f - flipZ)  - pivot.Z, 1.f)
	...
	*/
	...
	//rotate order: x,y,z
	float flipX = MEDUSA_FLAG_HAS(flip, FlipMask::X) ? -1.f : 1.f;
	float flipY = MEDUSA_FLAG_HAS(flip, FlipMask::Y) ? -1.f : 1.f;
	float flipZ = MEDUSA_FLAG_HAS(flip, FlipMask::Z) ? -1.f : 1.f;


	float oneMinusflipX = 1.f - flipX;
	float oneMinusflipY = 1.f - flipY;
	float oneMinusflipZ = 1.f - flipZ;

	//-flipPivot*flip*(flipPivot)*(-pivot)*scale:
	Matrix4 result(scale.X*flipX, 0.f, 0.f, 0.f,
		0.f, scale.Y*flipY, 0.f, 0.f,
		0.f, 0.f, scale.Z*flipZ, 0.f,
		flipPivot.X*(oneMinusflipX)*scale.X - scale.X*pivot.X, flipPivot.Y*(oneMinusflipY)*scale.Y - scale.Y*pivot.Y, flipPivot.Z*(oneMinusflipZ)*scale.Z - scale.Z*pivot.Z, 1.f);

	if (skewXY != Rotation2F::Zero)
	{
		float skewX = Math::Tan(skewXY.X);
		float skewY = Math::Tan(skewXY.Y);
		...
		skewX, 1, 0.f, 0.f,
			...
			0.f, 0.f, 0.f, 1.f);
		*/
		result.M12 = result.M11*skewY;
		result.M21 = result.M22*skewX;

		float m41 = result.M41;
		float m42 = result.M42;

		result.M41 += m42*skewX;
		result.M42 += m41*skewY;
	}
...
Matrix4 Matrix4::CreateWorldWithoutAnchor(const Scale3F& scale,const Rotation2F& skewXY, const Rotation3F& rotation, const Point3F& translation, FlipMask flip /*= FlipMask::None*/)
{
	//matrix order: flip*scale*skew*rotation*translation
