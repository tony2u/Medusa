...
namespace Siren
{
	...
	void Serialize(TWriter& writer, const Ellipse<T>& obj)
	{
		...
		writer.template OnField<Point2<T>, false>(StringRef("Center", 6), 0, obj.Center);
		writer.template OnField<Size2<T>, false>(StringRef("Radius", 6), 1, obj.Radius);
		writer.OnStructEnd();
	}
	...
	bool DeserializeTo(TReader& reader, Ellipse<T>& outObj)
	{
		...
		reader.template OnField<Point2<T>, false>(StringRef("Center", 6), 0, outObj.Center);
		reader.template OnField<Size2<T>, false>(StringRef("Radius", 6), 1, outObj.Radius);
		reader.OnStructEnd();
