...
public:
	...
	DateTime operator-(const TimeSpan& timeSpan)const{return DateTime(mTotalSeconds-timeSpan.TotalSeconds());}
	DateTime operator+(const TimeSpan& timeSpan)const{return DateTime(mTotalSeconds+timeSpan.TotalSeconds());}
	DateTime& operator-=(const TimeSpan& timeSpan){mTotalSeconds-=timeSpan.TotalSeconds();return *this;}
	DateTime& operator+=(const TimeSpan& timeSpan){mTotalSeconds+=timeSpan.TotalSeconds();return *this;}
private:
	...
	time_t mTotalSeconds;//seconds from 1970,always UTC time
	tm mDate;
	bool mIsUTC;
