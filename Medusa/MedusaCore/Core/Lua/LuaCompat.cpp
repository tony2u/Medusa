...
#define LUA_PACKAGE_KEY "_MEDUSA_COMPAT"
...
static  void _lua_pushpackagetable(lua_State* L)
{
...
lua_Unsigned lua_tounsignedx(lua_State* L, int i, int* is_num)
{
	lua_Unsigned result;
...
lua_Number lua_tonumberx(lua_State* L, int i, int* is_num)
{
...
lua_Integer lua_tointegerx(lua_State* L, int i, int* is_num)
{
...
static  int _luaL_countlevels(lua_State* L)
{
	lua_Debug ar;
	int li = 1, le = 1;
	/* find an upper bound */
...
static  int _luaL_findfield(lua_State* L, int objidx, int level)
{
...
static  int _luaL_pushglobalfuncname(lua_State* L, lua_Debug* ar)
{
...
static  void _luaL_pushfuncname(lua_State* L, lua_Debug* ar)
{
...
#ifdef WIN32
		char tempBuffer[256];
		strerror_s(tempBuffer, err);
		if (fname)
		{
			lua_pushfstring(L, "%s: %s", fname, tempBuffer);
		}
		else
		{
