...
namespace Siren
{
	...
	static MemoryData Serialize(const T& obj)
	{
		...
		if (SirenSchemaSerializer::Visit(writer, obj))
		{
	...
	template <class TSizeCounter, typename T>
	static MemoryData ByteSize(const T& obj)
	{
		TSizeCounter counter;
		SirenSchemaSerializer::Visit(counter, obj);
		return counter.Size();
	}
	...
	static bool SerializeLua(const T& obj, LuaState& state)
	{
		...
		return SirenSchemaSerializer::Visit(writer, obj);
	}
