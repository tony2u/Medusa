...
+ (NSString *) stringFromAddress: (const struct sockaddr *) address
{
	if(address && address->sa_family == AF_INET) {
		...
		return [NSString stringWithFormat:@"%@:{}", [NSString stringWithUTF8String:inet_ntoa(sin->sin_addr)], ntohs(sin->sin_port)];
	}
...
- (NSString *) getIPAddressForHost: (NSString *) theHost
{
    ...
    if (!host) {herror("resolv"); return nullptr; }
...
- (BOOL) hostAvailable: (NSString *) theHost
{
	...
	SCNetworkReachabilityRef defaultRouteReachability = SCNetworkReachabilityCreateWithAddress(nullptr, (struct sockaddr *)&address);
...
- (void) privateShowAlert: (id) formatstring,...
{
	...
	va_end(arglist);
    [UIAlertController alertControllerWithTitle:outstring message:nil preferredStyle:UIAlertControllerStyleAlert];
...
- (BOOL) scheduleReachabilityWatcher: (id) watcher
{
	...
	*/
	...
	SCNetworkReachabilityContext context = {0, (__bridge void*)watcher, nullptr, nullptr, nullptr};
	if(SCNetworkReachabilitySetCallback(reachability, ReachabilityCallback, &context)) 
	{
		if(!SCNetworkReachabilityScheduleWithRunLoop(reachability, CFRunLoopGetCurrent(), kCFRunLoopCommonModes)) 
		{
			...
			SCNetworkReachabilitySetCallback(reachability, nullptr, nullptr);
			return NO;
		}
	} 
	else 
