...
{ 
	...
	CFStreamClientContext		ctxt = {0, nullptr, nullptr, nullptr, nullptr};
	...
	if (addr != INADDR_NONE) { 
		...
		addressData = CFDataCreate(nullptr, (UInt8 *)&sin, sizeof(sin)); 
...
static void CleanupAfterWAAN(MyStreamInfoPtr myInfoPtr)
{
	assert(myInfoPtr != nullptr);
	if (myInfoPtr->isClientSet)
	{
		CFWriteStreamSetClient(myInfoPtr->wStreamRef, 0, nullptr, nullptr);
