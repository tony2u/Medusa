...
public:
	...
	T Align(T val)const
	{
		if (Math::Abs(MovementFromMin(val))>Math::Abs(MovementToMax(val)))
		{
			return Max;
		}
		return Min;
...
namespace Siren
{
	...
	void Serialize(TWriter& writer, const Range<T>& obj)
	{
		...
		writer.template OnField<T, false>(StringRef("Min", 3), 0, obj.Min);
		writer.template OnField<T, false>(StringRef("Max", 3), 1, obj.Max);
		writer.OnStructEnd();
	}
	...
	bool DeserializeTo(TReader& reader, Range<T>& outObj)
	{
		...
		reader.template OnField<T, false>(StringRef("Min", 3), 0, outObj.Min);
		reader.template OnField<T, false>(StringRef("Max", 3), 1, outObj.Max);
		reader.OnStructEnd();
