...
static int lf_lfind_match(LF_SLIST * volatile *head,
                           ...
                           uint rand_val)
{
  ...
  */
  res= lf_lfind_match(el, rev_hashnr | 1, UINT_MAX32, match, &cursor, pins);
  ...
  if (! res && hashnr != 0)
  {
    ...
    */
    ...
    if (unlikely(!el))
      return MY_ERRPTR;
    res= lf_lfind_match(el, 1, rev_hashnr, match, &cursor, pins);
  }
